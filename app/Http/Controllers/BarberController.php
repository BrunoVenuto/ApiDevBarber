...
class BarberController extends Controller
{
    ...
    public function createRandom() {
        ...
        for($q=0;$q<3;$q++) {
            $names = ['Bruno ', 'Davi ', 'Duda '];
            $lastnames = [' Tiago', ' Venuto', ' Viana'];

            $servicos = ['Corte', 'Pintura',];
            $servicos2 = ['cabelo', 'barba'];

            $depos = [
                'Muito bom! Adorei o serviço, profissional muito educado',
                'Excelente, recomendo!!',
                'Top dos tops'
            ];
            ...
            $newBarber->avatar = rand(1, 4).'.png';//estava com (1, 4)
            $newBarber->stars = rand(2, 4).'.'.rand(0, 9);
            $newBarber->latitude = '-19.4'.rand(0,9).'4679';
            $newBarber->longitude = '-44.2'.rand(0,9).'2477';
            $newBarber->save();

            $ns = rand(2, 6);//estava (3, 6)

            for($w=0;$w<4;$w++) {
                $newBarberPhoto = new BarberPhotos();
                $newBarberPhoto->id_barber = $newBarber->id;
                $newBarberPhoto->url = rand(1, 5).'.png';//estava com (1, 5)
                $newBarberPhoto->save();
            }
    ...
    */
    ...
    public function list(Request $request) {
        ...
        if(!empty($city)) {
            ...
            if(count($res['results']) > 0) {
                $city = $res['results'][0]['formated_address'];
            }
        }else {
            $lat = '-19.4679';
            $lng = '-44.2477';
    ...
    public function setAppointment($id, Request $request) {
        ...
        // 1. verificar se o serviço do barber existe
        ...
        if($barberservice) {
            ...
            if(strtotime($apDate) > 0) {
                // 3. verificar se o barber ja possui agenda neste dia/hora
                $apps = UserAppointment::select()
                    ->where('id_barber', $id)
                    ->where('ap_datetime', $apDate)
                ->count();
                if($apps === 0) {
                    // 4. verificar se barber atende nessa dta
